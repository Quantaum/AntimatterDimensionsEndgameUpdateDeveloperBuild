...
export function gameLoop(passedDiff, options = {}) {
  ...
  if (!Enslaved.isReleaseTick && !fixedSpeedActive) {
    ...
    if (Enslaved.isStoringGameTime && !fixedSpeedActive) {
      ...
      const totalTimeFactor = EndgameUpgrade(7).isBought
        ? Decimal.max(getGameSpeedupFactor([GAME_SPEED_EFFECT.FIXED_SPEED, GAME_SPEED_EFFECT.TIME_GLYPH, GAME_SPEED_EFFECT.BLACK_HOLE,
        GAME_SPEED_EFFECT.SINGULARITY_MILESTONE, GAME_SPEED_EFFECT.CELESTIAL_MATTER]), player.records.thisEndgame.peakGameSpeed)
        : getGameSpeedupFactor([GAME_SPEED_EFFECT.FIXED_SPEED, GAME_SPEED_EFFECT.TIME_GLYPH, GAME_SPEED_EFFECT.BLACK_HOLE,
